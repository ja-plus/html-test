...
impl Solution {
     ...
     * @return int整型
     */
    pub fn InversePairs(&self, data: Vec<i32>) -> i32 {
        // write code here
        let count = self.split(&data, 0, data.len() - 1);

        (count % 1000000007) as i32
    }

    fn split(&self, data: &Vec<i32>, left: usize, right: usize) -> usize {
    ...
    fn merge(&self, data: &Vec<i32>, left: usize, mid: usize, right: usize) -> usize {
        let mut i = left;
        let mut j = mid + 1;
        let mut count = 0;
        // while i <= mid && j <= right {
