<script>
  ...
  let {virtual,style,columns,dataSource} = $props();
  ...
  let dataSourceCopy = $derived([...dataSource]);
  /** 数据量大于2页才开始虚拟滚动*/

  /** 虚拟滚动展示的行数 */
  let virtual_pageSize =$derived(
    Math.ceil(virtualScroll.containerHeight / virtualScroll.rowHeight) + 1) // 这里最终+1，因为headless=true无头时，需要上下各预渲染一行。
  let virtual_on = $derived(virtual && dataSourceCopy.length > virtual_pageSize * 2);

  /** 虚拟滚动展示的行 */
  let virtual_dataSourcePart = $derived(virtual_on
    ? dataSourceCopy.slice(
        virtualScroll.startIndex,
        virtualScroll.startIndex + virtual_pageSize,
      )
    : dataSourceCopy);
  /** 虚拟表格定位下边距*/
  let virtual_offsetBottom = $derived(virtual_on
    ? (dataSourceCopy.length -
        virtualScroll.startIndex -
        virtual_dataSourcePart.length) *
  ...
  function initVirtualScrollY(height) {
    if (virtual_on) {
      virtualScroll.containerHeight =
        typeof height === "number" ? height : tableContainer?.offsetHeight;
